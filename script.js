...
const renderSinglePartyById = async (id) => {
  try {
    ...
    partyDetailsElement.innerHTML = `
        ...
        `;
    let party_div = partyContainer.getElementsByClassName("party")
    //partyContainer.appendChild(partyDetailsElement);
    for(item in party_div){
      try {
        let item_id = party_div[item].getElementsByClassName("details-button")[0].attributes["data-id"].value; 
        if(Number( item_id) === Number( id)){
          party_div[item].appendChild(partyDetailsElement);
          break;
        }
      } catch (error) {
        console.log("details-button does not exist!: {}",error);
      }
    };
  ...
  try {
    ...
    parties.forEach((party) => {
      ...
      detailsButton.addEventListener('click', async (event) => {
        ...
        // your code here
        let event_id = event.target.dataset.id;
        //let party = await getPartyById(event_id);
        await renderSinglePartyById(event_id);
      });
